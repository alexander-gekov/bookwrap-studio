...
export default function Home() {
  ...
  const compose = useCallback(async () => {
    ...
    ctx.fillStyle = "rgba(7,13,20,.48)"; ctx.fillRect(dims.bleed, dims.bleed, dims.trimW, dims.trimH);
    const spineX = dims.bleed + dims.trimW; ctx.fillStyle = "rgba(7,13,20,.16)"; ctx.fillRect(spineX, 0, dims.spineW, dims.totalH);
    if (blurb.trim()) {
      ctx.save(); const pad = Math.max(38, dims.trimW * .1), fontSize = Math.max(22, dims.trimW * .036);
      ctx.fillStyle = "#fffdf4"; ctx.font = `500 ${fontSize}px Arial`; ctx.textBaseline = "top";
      const lines: string[] = []; let line = "";
      for (const word of blurb.trim().split(/\s+/)) { const next = line ? `${line} ${word}` : word; if (ctx.measureText(next).width > dims.trimW - pad * 2 && line) { lines.push(line); line = word; } else line = next; }
      if (line) lines.push(line); lines.slice(0, 10).forEach((text, i) => ctx.fillText(text, dims.bleed + pad, dims.bleed + pad + i * fontSize * 1.45)); ctx.restore();
    }
    if (dims.spineW > 34) { ctx.save(); ctx.translate(spineX + dims.spineW / 2, dims.totalH / 2); ctx.rotate(Math.PI / 2); ctx.fillStyle = "#fffdf4"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.font = `700 ${Math.min(dims.spineW * .34, 44)}px Arial`; ctx.fillText(title.toUpperCase(), 0, 0, dims.trimH * .72); ctx.restore(); }
    ...
    return canvas;
  }, [blurb, config.dpi, coverUrl, dims, generatedUrl, title]);
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <section className="preview-panel">
        ...
        <div className="stage"><div className="stage-labels"><span>BACK</span><span>SPINE</span><span>FRONT</span></div><div className={`cover-spread ${!ready ? "empty-spread" : ""}`} style={{ gridTemplateColumns: `${config.width}fr ${config.spine}fr ${config.width}fr` }}>
